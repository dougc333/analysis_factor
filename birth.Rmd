...
<h1>Do not put all Xn variables in LM!! Why this is not a good idea? </h1>
```{r}
...
```
OK the m_bwt_res<-resid(m_bwt) model which is birth weight in pounds regressed on birth weight in ounces shows a lot
of skew. This is funny. Is same as y=Constant*y. What is this regression? 
This matches the gestation is the first one, seems the residuals are normal in this predictor. 
...
```
What is the difference between the correlation and covariance? Correlation close to 1 or -1 means highly correlated and
a 0 means not correlated. Looks like halfway so correlated
```{r}
...
cor(corr_data)
```
When you see a 1.00000 or high value off the diagonal. .93 is high!!! del_wgt and pre_wgt same. pre_wgt and bmi same
Those 3 closely correlated. Gestattion not related to those 3. wt_gain also good like gestation. bwt and bwt_pnds same. Go back and look at residuals of wt_gain. wg_gain has bad residuals. Funny. 
